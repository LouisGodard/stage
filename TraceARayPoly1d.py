...
if analytic:
    ...
    projectionZ=lambdify((R.x,R.y,R.z), projectionZ, "numpy")
    ...
    projectionZ=lambda x,y,z:np.float64(inter.splev(z,tck2,ext=2,der=1))
    norm=lambda x,y,z:np.abs(projectionZ(x,y,z))
    laplacian=lambda x,y,z:np.abs(np.float64(inter.splev(z,tck2,ext=2,der=2)))
...
d=lambda x,y,z:delta*norm(x,y,z)/laplacian(x,y,z)+delta/100*(1/(1+delta*norm(x,y,z)/laplacian(x,y,z)))
