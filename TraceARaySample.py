...
def gradOrientation(z0):
    ...
    correction=np.sign(grad[indice(z0,zProfil)])#correction =1 if value >0 and -1 if
    #value<0 (value<0 means that the gradient and the z axis are opposed)
    if correction==0:#In the case where the gradient is null we change nothing
        correction=1
    return (correction)

def indice(z,zProfil):
    """Return the indice which make zProfil[indice] the closest of z """

    if z<zProfil[0] or z>zProfil[-1]:
        raise ValueError("""you need to increase the range of your speed profil
or you should reduce the angle of your rays""")
    elif z<zProfil[1]/2: #for the first indice
        return (0)
    elif z>(zProfil[-1]+zProfil[-2])/2: #for the last
        return (-1)
    else:
        for i in range (1,zProfil.size-1,1):
            if z>=(zProfil[i]+zProfil[i-1])/2 and z<(zProfil[i+1]+zProfil[i])/2:
            #we check if it is between the middles of the z of zProfil
                return(i)
...
grad=np.gradient(speedProfil[1,]**(-2),speedProfil[0,])#grad is used in gradOrientation
sndDeriv=np.gradient(grad,speedProfil[0,])
laplacian=lambda x,y,z:sndDeriv[indice(z,zProfil)]
norm=lambda x,y,z:grad[indice(z,zProfil)]
